...
int main(int argc, char **argv) {
    ...
    logger->info("{{\n"
        ...
        "  \"n_round\": {}\n"
        "}}",
        data_type, data_path, query_path, groundtruth_path, sptag_server_addr, sptag_server_port, vector_dim, K, search_threads, n_splits, n_split_repeat, metric_str, window_size, n_repeat, stride, n_round);
    if (data_type == "float") {
        experiment_benchmark<float>(data_type, data_path, query_path, groundtruth_path, K, search_threads, n_splits, n_split_repeat, metric, sptag_server_addr, sptag_server_port, vector_dim, window_size, n_repeat, stride, n_round);
    } else if (data_type == "int8") {
        experiment_benchmark<int8_t>(data_type, data_path, query_path, groundtruth_path, K, search_threads, n_splits, n_split_repeat, metric, sptag_server_addr, sptag_server_port, vector_dim, window_size, n_repeat, stride, n_round);
    } else if (data_type == "uint8") {
        experiment_benchmark<uint8_t>(data_type, data_path, query_path, groundtruth_path, K, search_threads, n_splits, n_split_repeat, metric, sptag_server_addr, sptag_server_port, vector_dim, window_size, n_repeat, stride, n_round);
    } else {
